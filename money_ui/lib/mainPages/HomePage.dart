    ...
    return Scaffold(
      ...
      ListView(
        ...
        children: [
          Container(
            decoration:
                const BoxDecoration(image: DecorationImage(fit: BoxFit.fill, image: AssetImage('assets/background.png'))),
            child: Column(children: [
              ...
              Row(
                ...
                children: [
                  Padding(
                    padding: const EdgeInsets.only(left: 30, bottom: 20, top: 15),
                    child: Column(
                      // ignore: prefer_const_literals_to_create_immutables
                      children: [
                        const Text(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  ...
                                  style:
                                      const TextStyle(color: Colors.greenAccent, fontSize: 18, fontWeight: FontWeight.bold),
                                ),
                                Text(
...
class _CalendarWeeklyViewState extends State<CalendarWeeklyView> {
  ...
  @override
  Widget build(BuildContext context) {
    DateTime today = DateTime.fromMicrosecondsSinceEpoch(DateTime.now().microsecondsSinceEpoch);
      ...
      return SizedBox(
        ...
        child: Container(
            ...
            child: CupertinoSlidingSegmentedControl<int>(
                ...
                children: {
                  0: buildSegment(DateFormat.EEEE().format(today.subtract(const Duration(days: 8))).toString(),
                      DateFormat.d().format(today.subtract(const Duration(days: 8))).toString(), context),
                  1: buildSegment(DateFormat.EEEE().format(today.subtract(const Duration(days: 7))).toString(),
                      DateFormat.d().format(today.subtract(const Duration(days: 7))).toString(), context),
                  2: buildSegment(DateFormat.EEEE().format(today.subtract(const Duration(days: 6))).toString(),
                      DateFormat.d().format(today.subtract(const Duration(days: 6))).toString(), context),
                  3: buildSegment(DateFormat.EEEE().format(today.subtract(const Duration(days: 5))).toString(),
                      DateFormat.d().format(today.subtract(const Duration(days: 5))).toString(), context),
                  4: buildSegment(DateFormat.EEEE().format(today.subtract(const Duration(days: 4))).toString(),
                      DateFormat.d().format(today.subtract(const Duration(days: 4))).toString(), context),
                  5: buildSegment(DateFormat.EEEE().format(today.subtract(const Duration(days: 3))).toString(),
                      DateFormat.d().format(today.subtract(const Duration(days: 3))).toString(), context),
                  6: buildSegment(DateFormat.EEEE().format(today.subtract(const Duration(days: 2))).toString(),
                      DateFormat.d().format(today.subtract(const Duration(days: 2))).toString(), context),
                },
    ...
    return Column(
      children: [calendarBox(context), VerticalList(date: dates[groupValue!.toInt()])],
    );
  }
}
