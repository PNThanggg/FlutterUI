...
class KnobController extends ValueNotifier<KnobValue> {
  ...
  KnobController({
    ...
    this.precision = 2,
        ...
        assert(maximum >= initial && initial >= minimum, 'Initial value must be between minimum and maximum values.'),
        super(KnobValue(
          current: initial,
          minimum: minimum,
          maximum: maximum,
  ...
  double getAngleOfValue(double newValue) {
    ...
    var angle = (max - min) / (value.maximum - value.minimum) * (newValue - value.minimum) + min;
  ...
  double getValueOfAngle(double angle) {
    ...
    return (value.maximum - value.minimum) / (max - min) * (angle - min) + value.minimum;
  }
